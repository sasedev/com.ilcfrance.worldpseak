...
class SecurityController extends BaseController
{
            ...
            if ($securityDontRememberUsernameForm->isValid()) {
                ...
                if (null != $user) {
                    ...
                    $message = Swift_Message::newInstance()->setFrom($from, $fromName)
                        ...
                        ->setSubject($subject)
                        ->setBody($this->renderView('IlcfranceWorldspeakAdminSecurityBundle:Mail:dontRememberUsername.html.twig', $mvars), 'text/html');
                ...
                if (null != $user) {
                    ...
                    if (null == $user->getRecoveryExpiration() || $user->getRecoveryExpiration() < $now) {
                        ...
                        $message = Swift_Message::newInstance()->setFrom($from, $fromName)
                            ->setTo($user->getEmail(), $user->getFullname())
                            ->setSubject($subject)
                            ->setBody($this->renderView('IlcfranceWorldspeakAdminSecurityBundle:Mail:getPasswordResetLink.html.twig', $mvars), 'text/html');
            ...
            if (null != $user) {
                ...
                if (null == $user->getRecoveryExpiration() || $user->getRecoveryExpiration() < $now) {
                    ...
                    $message = Swift_Message::newInstance()->setFrom($from, $fromName)
                        ->setTo($user->getEmail(), $user->getFullname())
                        ->setSubject($subject)
                        ->setBody($this->renderView('IlcfranceWorldspeakAdminSecurityBundle:Mail:genNewPassword.html.twig', $mvars), 'text/html');
    ...
    public function myAvatarAction(Request $request)
    {
        ...
        $response->setContent($avatar->getFile()
            ->getBytes());
