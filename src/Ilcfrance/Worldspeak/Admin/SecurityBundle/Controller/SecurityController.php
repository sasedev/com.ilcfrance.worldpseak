<?php
namespace Ilcfrance\Worldspeak\Admin\SecurityBundle\Controller;

use Ilcfrance\Worldspeak\Shared\ResBundle\Controller\BaseController;
use Ilcfrance\Worldspeak\Admin\SecurityBundle\Form\SecurityDontRememberUsernameTForm;
use Ilcfrance\Worldspeak\Admin\SecurityBundle\Form\SecurityLostPasswordTForm;
use Ilcfrance\Worldspeak\Admin\SecurityBundle\Form\UserAvatarResizeTForm;
use Ilcfrance\Worldspeak\Admin\SecurityBundle\Form\UserAvatarTForm;
use Ilcfrance\Worldspeak\Admin\SecurityBundle\Form\UserEditEmailTForm;
use Ilcfrance\Worldspeak\Admin\SecurityBundle\Form\UserEditPasswordTForm;
use Ilcfrance\Worldspeak\Admin\SecurityBundle\Form\UserEditPreferedLocaleTForm;
use Ilcfrance\Worldspeak\Admin\SecurityBundle\Form\UserEditProfileTForm;
...
use Ilcfrance\Worldspeak\Admin\SecurityBundle\Form\LoginTForm;
...
class SecurityController extends BaseController
{
	...
	public function loginAction(Request $request)
	{
		...
		if ($this->isGranted('IS_AUTHENTICATED_FULLY')) {
			return $this->redirect($this->generateUrl('admin_security_profile'));
		}
		...
		return $this->render('IlcfranceWorldspeakAdminSecurityBundle:Security:login.html.twig', $this->getTwigVars());
	}
	...
	public function dontRememberUsernameAction(Request $request)
	{
		...
		if ($request->getMethod() == 'POST') {
			...
			if ($securityDontRememberUsernameForm->isValid()) {
				...
				$user = $em->getRepository('IlcfranceWorldspeakSharedDataBundle:Admin')->findOneBy(array(
					'email' => $email
				));
				...
				if (null != $user) {
					...
					$message = \Swift_Message::newInstance()->setFrom($from, $fromName)->setTo($user->getEmail(), $user->getFullname())->setSubject($subject)->setBody($this->renderView('IlcfranceWorldspeakAdminSecurityBundle:Mail:dontRememberUsername.html.twig', $mvars), 'text/html');

					$this->sendmail($message);

					$this->addFlash('success', $this->translate('_sec.dontRememberUsername_mail_username_sent', array(
						'%mail%' => $email
					)));
					...
					return $this->redirect($this->generateUrl('admin_security_lost_username'));
				}
			}
		}
		...
		return $this->render('IlcfranceWorldspeakAdminSecurityBundle:Security:dontRememberUsername.html.twig', $this->getTwigVars());
	}
	...
	public function lostPasswordAction(Request $request)
	{
			...
			if ($securityLostPasswordForm->isValid()) {
				...
				if (null != $user) {
					...
					if (null == $user->getRecoveryExpiration() || $user->getRecoveryExpiration() < $now) {
						...
						$user->setRecoveryCode(Admin::generateRandomChar(20, 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'));
						$em->persist($user);
						$em->flush();
						...
						$mvars['user'] = $user;
						$mvars['userPreferedLocale'] = $locale;
						$mvars['url'] = $this->generateUrl('admin_security_lost_genpassword', array(
							'id' => $user->getId(),
						...
						$message = \Swift_Message::newInstance()->setFrom($from, $fromName)->setTo($user->getEmail(), $user->getFullname())->setSubject($subject)->setBody($this->renderView('IlcfranceWorldspeakAdminSecurityBundle:Mail:getPasswordResetLink.html.twig', $mvars), 'text/html');

						$this->sendmail($message);

						$this->addFlash('success', $this->translate('_sec.lostPassword_mail_newpass_sent', array(
							'%mail%' => $user->getEmail()
						)));
					...
					return $this->redirect($this->generateUrl('admin_security_lost_password'));
				}
			}
		}
		...
		return $this->render('IlcfranceWorldspeakAdminSecurityBundle:Security:lostPassword.html.twig', $this->getTwigVars());
	}
	...
	public function genNewPasswordAction($id, $code, Request $request)
	{
		...
		$em = $this->getEntityManager();
		try {
			$user = $em->getRepository('IlcfranceWorldspeakSharedDataBundle:Admin')->find($id);
			...
			if (null != $user) {
				...
				if (null == $user->getRecoveryExpiration() || $user->getRecoveryExpiration() < $now) {
					...
					$user->setClearPassword(Admin::generateRandomChar(8, 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'));
					...
					$message = \Swift_Message::newInstance()->setFrom($from, $fromName)->setTo($user->getEmail(), $user->getFullname())->setSubject($subject)->setBody($this->renderView('IlcfranceWorldspeakAdminSecurityBundle:Mail:genNewPassword.html.twig', $mvars), 'text/html');

					$this->sendmail($message);

					$this->addFlash('success', $this->translate('_sec.genNewPassword_ok'));
				}
			} else {
	...
	public function myProfileAction(Request $request)
	{
		...
		if ($request->getMethod() == 'POST') {
			$logger = $this->getLogger();
			$logger->addError('passage en post');
			...
			if (isset($data['UserEditPasswordForm'])) {
				...
				if ($userEditPasswordForm->isValid()) {
					...
					if ($oldpassEncoded != $oldDbpass) {
						...
						$em->persist($user);
						$em->flush();
						$this->addFlash('success', 'profile.updatesuccess.password');
			...
			if (isset($data['UserEditEmailForm'])) {
				$tabActive = 2;
				$logger->addError('passage en post email');
				$userEditEmailForm->handleRequest($request);
				if ($userEditEmailForm->isValid()) {
					$em->persist($user);
					$em->flush();
					$this->addFlash('success', 'profile.updatesuccess.mail');
			...
			if (isset($data['UserEditProfileForm'])) {
				...
				if ($userEditProfileForm->isValid()) {
					$em->persist($user);
					$em->flush();
					$this->addFlash('success', 'profile.updatesuccess.advancedprofile');
			...
			if (isset($data['UserEditPreferedLocaleForm'])) {
				...
				if ($userEditPreferedLocaleForm->isValid()) {
					...
					$em->persist($user);
					$em->flush();
					$this->addFlash('success', 'profile.updatesuccess.preferedLocale');
			...
			if (isset($data['UserAvatarForm'])) {
				...
				if ($userAvatarForm->isValid()) {
					...
					return $this->render('IlcfranceWorldspeakAdminSecurityBundle:Security:resize_avatar.html.twig', $this->getTwigVars());
				} else {
					$this->addTwigVar('userAvatarForm', $userAvatarForm->createView());

					return $this->render('IlcfranceWorldspeakAdminSecurityBundle:Security:resize_avatar_error.html.twig', $this->getTwigVars());
				}
			}
			if (isset($data['UserAvatarResizeForm'])) {
				...
				if ($userAvatarResizeForm->isValid()) {
					...
					$em->flush();
					unlink($path);
					$this->addFlash('success', 'profile.updatesuccess.avatar');
				} else {
		...
		return $this->render('IlcfranceWorldspeakAdminSecurityBundle:Security:myprofile.html.twig', $this->getTwigVars());
	}
}
