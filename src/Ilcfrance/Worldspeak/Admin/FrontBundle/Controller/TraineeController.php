...
class TraineeController extends BaseController
{
     ...
     */
    public function listByCompanyAction(Request $request, $id, $page = 1)
    {
            ...
            if (null == $trainee) {
                ...
                if (null != $trainee->getEmail() && trim($trainee->getEmail()) != '') {
                    ...
                    $message = Swift_Message::newInstance()->setFrom($from, $fromName)
                        ->setTo($trainee->getEmail(), $trainee->getFullname())
                        ->setSubject($subject)
                        ->setBody($this->renderView('IlcfranceWorldspeakSharedResBundle:Mail:trainee.newpass.html.twig', $mvars), 'text/html');
