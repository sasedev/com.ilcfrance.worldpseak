...
class DefaultController extends BaseController
{
    ...
    /**
     * Techers Reset sentmail/new password
     *
     * @param Request $request
     *
     * @return RedirectResponse
    ...
    /**
     * Send mails to marqued Teachers
     *
     * @param Request $request
     *
     * @return RedirectResponse
                ...
                try {
                    if (null != $teacher->getEmail() && trim($teacher->getEmail()) != '') {
                        ...
                        $message = Swift_Message::newInstance()->setFrom($from, $fromName)
                            ->setTo($teacher->getEmail(), $teacher->getFullname())
                            ->setSubject($subject)
                            ->setBody($this->renderView('IlcfranceWorldspeakSharedResBundle:Mail:teacher.registration.html.twig', $mvars), 'text/html');
    ...
    /**
     * Trainees Reset sentmail/new password
     *
     * @param Request $request
     *
     * @return RedirectResponse
    ...
    /**
     * Send mails to marqued lTrainees
     *
     * @param Request $request
     *
     * @return RedirectResponse
                ...
                try {
                    if (null != $trainee->getEmail() && trim($trainee->getEmail()) != '') {
                        ...
                        $message = Swift_Message::newInstance()->setFrom($from, $fromName)
                            ...
                            ->setSubject($subject)
                            ->setBody($this->renderView('IlcfranceWorldspeakSharedResBundle:Mail:trainee.registration.html.twig', $mvars), 'text/html');
    ...
    private function importCompanies(PHPExcel $excelObj)
    {
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            if (null == $Company) {
                ...
                $Company->setName(strval($worksheet->getCellByColumnAndRow(1, $row)
                    ->getValue()));
                $Company->setService(strval($worksheet->getCellByColumnAndRow(2, $row)
                    ->getValue()));
                $Company->setAddress(strval($worksheet->getCellByColumnAndRow(3, $row)
                    ->getValue()));
                ...
                $Company->setPostalCode(strval($worksheet->getCellByColumnAndRow(5, $row)
                    ->getValue()));
                $Company->setTown(strval($worksheet->getCellByColumnAndRow(6, $row)
                    ->getValue()));
    ...
    private function importTeacher(\PHPExcel $excelObj)
    {
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            if (stripos($tNom, "(FAC)") !== false) {
                ...
                $Teacher2 = null;
                $tEmail = $this->normalize(trim(strval($worksheet->getCellByColumnAndRow(12, $row)
                    ->getValue())));
                if ($tEmail != "") {
                ...
                if (null == $Teacher && null == $Teacher2) {
                    ...
                    $Teacher->setLastName(trim(strval($worksheet->getCellByColumnAndRow(1, $row)
                        ->getValue())));
                    $Teacher->setFirstName(trim(strval($worksheet->getCellByColumnAndRow(2, $row)
                        ->getValue())));
                    ...
                    $Teacher->setPhone(trim(strval($worksheet->getCellByColumnAndRow(11, $row)
                        ->getValue())));
                    $Teacher->setMobile(trim(strval($worksheet->getCellByColumnAndRow(12, $row)
                        ->getValue())));
    ...
    private function importTrainees(\PHPExcel $excelObj)
    {
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            $Trainee2 = null;
            $tEmail = $this->normalize(strval($worksheet->getCellByColumnAndRow(10, $row)
                ->getValue()));
            if ($tEmail != "") {
            ...
            if (null == $Trainee && null == $Trainee2) {
                ...
                if (null != $Company) {
                    ...
                    $Trainee->setLastName(trim(strval($worksheet->getCellByColumnAndRow(1, $row)
                        ->getValue())));
                    $Trainee->setFirstName(trim(strval($worksheet->getCellByColumnAndRow(2, $row)
                        ->getValue())));
                    ...
                    $Trainee->setRegistermail(Trainee::REGISTERMAIL_NOTSENT);
                    $Trainee->setJob(trim(strval($worksheet->getCellByColumnAndRow(5, $row)
                        ->getValue())));
    ...
    private function importCourses(\PHPExcel $excelObj)
    {
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            if (null != $Cours) {
                ...
                if (null == $curTrainee) {
                    $logger->addNotice("Erreur lors de l'import du cours à la ligne " . intval($crLignesLues + 1) . ' Stagiaire inconnu : ' . strval($worksheet->getCellByColumnAndRow(10, $row)
                        ->getValue()));
                    $crError++;
                } else {
