				...
				if ($timeCreditAddForm->isValid()) {
					...
					if ($canAddNewtimeCredit) {
						...
						if ($timeCredit->getNotifyByMail() == TimeCredit::NOTIFYBYMAIL_SENT && null != $trainee->getEmail() && trim($trainee->getEmail()) != '') {
							...
							$message = \Swift_Message::newInstance()->setFrom($from, $fromName)->setTo($trainee->getEmail(), $trainee->getFullname())->setSubject($subject)->setBody($this->renderView('IlcfranceWorldspeakSharedResBundle:Mail:trainee.new_timeCredit.html.twig', $mvars), 'text/html');
