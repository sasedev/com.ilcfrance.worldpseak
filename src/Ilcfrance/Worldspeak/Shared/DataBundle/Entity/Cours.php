...
class Cours extends FCEventClass
{
	...
	{
		$end = new \DateTime();
		$end->setTimestamp($this->getDtStart()->getTimestamp());
	...
	/**
	 * (non-PHPdoc) @see
	 * \Ilcfrance\Worldspeak\Shared\DataBundle\Model\FCEventClass::getBackgroundColor()
	 *
	 * @return string
	 */
	...
	/**
	 * (non-PHPdoc) @see
	 * \Ilcfrance\Worldspeak\Shared\DataBundle\Model\FCEventClass::getBorderColor()
	 *
	 * @return string
	 */
	...
	/**
	 * (non-PHPdoc) @see
	 * \Ilcfrance\Worldspeak\Shared\DataBundle\Model\FCEventClass::getTextColor()
	 *
	 * @return string
	 */
			...
			if ($teacher->getUsername() != 'a.definir' && $teacher->getUsername() != 'variables.formateurs') {
				...
				if (null != $courses) {
					...
					foreach ($courses as $cours) {
						...
						$dtStart1->setTimestamp($cours->getDtStart()->getTimestamp());

						$dtStart2 = new \DateTime('now');
						$dtStart2->setTimestamp($cours->getDtStart()->getTimestamp());
	...
	public function updateBuggy()
	{
		...
		if (null == $this->getPhone() || trim($this->getPhone()) == '') {
			if (null != $this->getTeacher() && null != $this->getTeacher()->getCoursPhone() && trim($this->getTeacher()->getCoursPhone()) != '') {
				$this->setPhone($this->getTeacher()->getCoursPhone());
			} else {
		...
		if ($this->getDtStart() > $now) {
			...
			$before24h->setTimestamp($this->getDtStart()->getTimestamp());
			...
			if (null == $traineeNotifB24h) {
				...
				$traineeNotifB24h->setTrainee($this->getTimeCredit()->getTrainee());
		...
		$after1h->setTimestamp($this->getDtEnd()->getTimestamp());
		...
		$after2h->setTimestamp($this->getDtEnd()->getTimestamp());
