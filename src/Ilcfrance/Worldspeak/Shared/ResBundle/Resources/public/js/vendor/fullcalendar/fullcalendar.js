...
function AgendaView(element, calendar, viewName) {
	...
	function renderAnnotations(annotations) {
		...
		for (var i=0; i < annotations.length; i++) {
			...
			dtStart.setTime(ann.start*1000);
			var dtEnd = new Date();
			dtEnd.setTime(ann.end*1000);
			
			//alert(dtStart+' '+this.start+' '+this.start+' '+this.start+' '+this.start+' '+this.start+' '+this.start+' '+this.start+' '+this.start+' '+this.start+' '+this.start+' '+this.start);
			
			
			if (dtStart >= this.start && dtEnd <= this.end) {
				//alert(dtStart+' / '+dtEnd + '    '+this.start+' / '+this.end);
				
				var top = timePosition(dtStart, dtStart);
				var bottom = timePosition(dtEnd, dtEnd);
				var height = bottom - top;
				var dayIndex = dayDiff(dtStart, t.visStart);
				...
				var colors = '';
				if (ann.textColor) {
					colors = 'color:' + ann.textColor + ';';
				}
				if (ann.backgroundColor) {
					colors += 'background:' + ann.backgroundColor + ';';
				}
