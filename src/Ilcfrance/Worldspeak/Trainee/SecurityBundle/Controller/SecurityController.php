            ...
            if ($securityDontRememberUsernameForm->isValid()) {
                ...
                if (null != $user) {
                    ...
                    $message = Swift_Message::newInstance()->setFrom($from, $fromName)
                        ...
                        ->setSubject($subject)
                        ->setBody($this->renderView('IlcfranceWorldspeakTraineeSecurityBundle:Mail:dontRememberUsername.html.twig', $mvars), 'text/html');
                ...
                if (null != $user) {
                    ...
                    if (null == $user->getRecoveryExpiration() || $user->getRecoveryExpiration() < $now) {
                        ...
                        $message = Swift_Message::newInstance()->setFrom($from, $fromName)
                            ->setTo($user->getEmail(), $user->getFullname())
                            ->setSubject($subject)
                            ->setBody($this->renderView('IlcfranceWorldspeakTraineeSecurityBundle:Mail:getPasswordResetLink.html.twig', $mvars), 'text/html');
            ...
            if (null != $user) {
                ...
                if (null == $user->getRecoveryExpiration() || $user->getRecoveryExpiration() < $now) {
                    ...
                    $message = Swift_Message::newInstance()->setFrom($from, $fromName)
                        ->setTo($user->getEmail(), $user->getFullname())
                        ->setSubject($subject)
                        ->setBody($this->renderView('IlcfranceWorldspeakTraineeSecurityBundle:Mail:genNewPassword.html.twig', $mvars), 'text/html');
