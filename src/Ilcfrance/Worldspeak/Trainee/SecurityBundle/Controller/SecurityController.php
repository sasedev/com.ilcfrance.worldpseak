<?php
namespace Ilcfrance\Worldspeak\Trainee\SecurityBundle\Controller;

use Ilcfrance\Worldspeak\Shared\ResBundle\Controller\BaseController;
use Ilcfrance\Worldspeak\Trainee\SecurityBundle\Form\SecurityDontRememberUsernameTForm;
use Ilcfrance\Worldspeak\Trainee\SecurityBundle\Form\SecurityLostPasswordTForm;
use Ilcfrance\Worldspeak\Trainee\SecurityBundle\Form\UserAvatarResizeTForm;
use Ilcfrance\Worldspeak\Trainee\SecurityBundle\Form\UserAvatarTForm;
use Ilcfrance\Worldspeak\Trainee\SecurityBundle\Form\UserEditEmailTForm;
use Ilcfrance\Worldspeak\Trainee\SecurityBundle\Form\UserEditPasswordTForm;
use Ilcfrance\Worldspeak\Trainee\SecurityBundle\Form\UserEditPreferedLocaleTForm;
use Ilcfrance\Worldspeak\Trainee\SecurityBundle\Form\UserEditProfileTForm;
...
use Ilcfrance\Worldspeak\Trainee\SecurityBundle\Form\LoginTForm;
...
class SecurityController extends BaseController
{
	...
	public function loginAction(Request $request)
	{
		...
		if ($this->isGranted('IS_AUTHENTICATED_FULLY')) {
			return $this->redirect($this->generateUrl('trainee_security_profile'));
		}
		...
		return $this->render('IlcfranceWorldspeakTraineeSecurityBundle:Security:login.html.twig', $this->getTwigVars());
	}
	...
	public function dontRememberUsernameAction(Request $request)
	{
		...
		if ($request->getMethod() == 'POST') {
			...
			if ($securityDontRememberUsernameForm->isValid()) {
				...
				$user = $em->getRepository('IlcfranceWorldspeakSharedDataBundle:Trainee')->findOneBy(array(
					'email' => $email
				));
				...
				if (null != $user) {
					...
					$message = \Swift_Message::newInstance()->setFrom($from, $fromName)->setTo($user->getEmail(), $user->getFullname())->setSubject($subject)->setBody($this->renderView('IlcfranceWorldspeakTraineeSecurityBundle:Mail:dontRememberUsername.html.twig', $mvars), 'text/html');
					...
					return $this->redirect($this->generateUrl('trainee_security_login'));
				} else {
					$this->addFlash('error', $this->translate('_sec.dontRememberUsername_notfound', array(
						'%mail%' => $email
					)));

					return $this->redirect($this->generateUrl('trainee_security_lost_username'));
				}
			}
		}
		...
		return $this->render('IlcfranceWorldspeakTraineeSecurityBundle:Security:dontRememberUsername.html.twig', $this->getTwigVars());
	}
	...
	public function lostPasswordAction(Request $request)
	{
			...
			if ($securityLostPasswordForm->isValid()) {
				...
				if (null != $user) {
					...
					if (null == $user->getRecoveryExpiration() || $user->getRecoveryExpiration() < $now) {
						...
						$user->setRecoveryCode(Trainee::generateRandomChar(20, 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'));
						$em->persist($user);
						$em->flush();
						...
						$mvars['user'] = $user;
						$mvars['userPreferedLocale'] = $locale;
						$mvars['url'] = $this->generateUrl('trainee_security_lost_genpassword', array(
							'id' => $user->getId(),
						...
						$message = \Swift_Message::newInstance()->setFrom($from, $fromName)->setTo($user->getEmail(), $user->getFullname())->setSubject($subject)->setBody($this->renderView('IlcfranceWorldspeakTraineeSecurityBundle:Mail:getPasswordResetLink.html.twig', $mvars), 'text/html');
					...
					return $this->redirect($this->generateUrl('trainee_security_lost_password'));
				}
			}
		}
		...
		return $this->render('IlcfranceWorldspeakTraineeSecurityBundle:Security:lostPassword.html.twig', $this->getTwigVars());
	}
	...
	public function genNewPasswordAction($id, $code, Request $request)
	{
		...
		$em = $this->getEntityManager();
		try {
			$user = $em->getRepository('IlcfranceWorldspeakSharedDataBundle:Trainee')->find($id);
			if (null != $user) {
				$now = new \DateTime('now');
				if (null == $user->getRecoveryExpiration() || $user->getRecoveryExpiration() < $now) {
					...
					$user->setClearPassword(Trainee::generateRandomChar(8, 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'));
					...
					$message = \Swift_Message::newInstance()->setFrom($from, $fromName)->setTo($user->getEmail(), $user->getFullname())->setSubject($subject)->setBody($this->renderView('IlcfranceWorldspeakTraineeSecurityBundle:Mail:genNewPassword.html.twig', $mvars), 'text/html');
	...
	public function myProfileAction(Request $request)
	{
		...
		if ($request->getMethod() == 'POST') {
			...
			if (isset($data['UserAvatarForm'])) {
				...
				if ($userAvatarForm->isValid()) {
					...
					return $this->render('IlcfranceWorldspeakTraineeSecurityBundle:Security:resize_avatar.html.twig', $this->getTwigVars());
				} else {
					$this->addTwigVar('userAvatarForm', $userAvatarForm->createView());

					return $this->render('IlcfranceWorldspeakTraineeSecurityBundle:Security:resize_avatar_error.html.twig', $this->getTwigVars());
				}
			}
		...
		return $this->render('IlcfranceWorldspeakTraineeSecurityBundle:Security:myprofile.html.twig', $this->getTwigVars());
	}
}
