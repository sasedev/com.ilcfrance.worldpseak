...
class CoursController extends BaseController
{
    ...
    public function downloadAction(Request $request, $id)
    {
        ...
        try {
            ...
            $pdf->SetTitle($title . ' - ' . $cours->getTimeCredit()
                ->getTrainee()
                ->getFullname());
            $pdf->SetSubject($title . ' - ' . $cours->getTimeCredit()
                ->getTrainee()
                ->getFullname());

            $pdf->SetHeaderData(null, 0, 'ILC WorldSpeak - ' . $title, $cours->getTimeCredit()
                ->getTrainee()
                ->getFullname(), array(
                0,
                32,
            ...
            $filename = $this->normalize('ILCWorldSpeak - ' . $title . ' - ' . $cours->getTimeCredit()
                ->getTrainee()
                ->getFullname());
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            try {
                ...
                foreach ($myCourses as $cCours) {
                    ...
                    $dtStart1->setTimestamp($cCours->getDtStart()
                        ->getTimestamp());

                    $dtStart2 = new DateTime('now');
                    $dtStart2->setTimestamp($cCours->getDtStart()
                        ->getTimestamp());
                    $dtStart2 = $dtStart2->modify('+30 minutes');

                    $dtStart3 = new DateTime('now');
                    $dtStart3->setTimestamp($cCours->getDtStart()
                        ->getTimestamp());
    ...
    public function ajaxEditAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            try {
                ...
                if ($currentUser->getId() != $cours->getTimeCredit()
                    ->getTrainee()
                    ->getId()) {
                    $traineeLog = new TraineeLog();
                    $traineeLog->setTrainee($currentUser);
    ...
    public function deleteAction(Request $request, $id)
    {
        ...
        try {
            ...
            if (null == $cours) {
                ...
                $em->persist($traineeLog);
                $em->flush();
            } elseif ($currentUser->getId() != $cours->getTimeCredit()
                ->getTrainee()
                ->getId()) {
