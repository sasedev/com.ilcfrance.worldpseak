        ...
        try {
            ...
            if (null != $coursDocument) {
                if (null != $coursDocument->getTraineeFile()) {
                    ...
                    $response->headers->set('Content-type', $coursDocument->getTraineeFile()
                        ->getMimeType());
                    $response->headers->set('Content-Disposition', 'attachment; filename="' . $coursDocument->getTraineeFile()
                        ->getFilename() . '"');

                    $response->headers->set('Content-length', $coursDocument->getTraineeFile()
                        ->getLength());
                    // Send headers before outputting anything
                    $response->sendHeaders();

                    $response->setContent($coursDocument->getTraineeFile()
                        ->getFile()
                        ->getBytes());
